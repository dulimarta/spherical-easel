  ...
  <v-card class="cards">
        ...
        <v-row>
          <v-col cols="auto">
            <v-btn
              icon
              size="small"
              :disabled="toolPick === null || modelValue.length >= 6"
    ...
    <v-card-text style="height: 100%">
      <v-list class="secondaryList" density="compact">
        <v-list-item v-for="(item,pos) in prettyList" :key="item.action"
        @click="selectedPos = pos">
          ...
          <v-list-item-title v-text="t(item.displayedName)"></v-list-item-title>
        </v-list-item>
      </v-list>
    </v-card-text>
  </v-card>
...
function addToolToFavorites(): void {
  if (props.toolPick) props.modelValue.push(props.toolPick);
  emit("tool-added", props.id);
}

function removeToolFromFavorites(): void {
  if (selectedPos.value !== null) {
    const victimTool = props.modelValue[selectedPos.value]
    props.modelValue.splice(selectedPos.value, 1)
