...
describe("SENoduleItem.vue", () => {
  ...
  const runTest = async (
    ...
    expect(w.props("node").name).toContain(se.name);
    const n = w.find(".node");
    ...
    for (let k = 1; k <= numberOfChildren; k++) { //  k =1 working k = 2 // not working
    ...
    if (isPlottable) {
      const w = createWrapper(TestComponent, {
        propsData: { node: se },
        stubs: {
          SENoduleItem: SENoduleItem,
          SESegmentLength: SESegmentLength,
          SEPointDistance: SEPointDistance,
          SESlider: SESlider,
          SEAngleMarker: SEAngleMarker,
          SEIntersectionPoint: SEIntersectionPoint,
          SEExpression: SEExpression,
          SECalculation: SECalculation,
          SENodule: SENodule,
          SEPoint: SEPoint,
          SELabel: SELabel,
          SELine: SELine,
      ...
      await runTest(w, se, true);
    }
      console.debug(n.html())
      const showhide = w.find("#_test_toggle_visibility");
  ...
  it("displays points with label, glows on mouseover/leave", async () => {
    ...
    aPoint.label = label;
    const wrapper = createWrapper(TestComponent,store, {
      mountOptions: {
