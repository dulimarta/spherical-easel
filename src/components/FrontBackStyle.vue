    ...
    <div v-show="showMoreLabelStyles">
      ...
      <InputGroup
        ...
        <!-- The dash property slider -->
          ...
          @change="
            updateLocalGapDashVariables(styleOptions, styleOptions?.dashArray ?? []);
...
const {selectedSENodules, oldStyleSelections, styleSavedFromPanel } = storeToRefs(seStore)
const { styleOptions, dataAgreement, forceDataAgreement, selectionCount,
  hasStyle, conflictingProps, angleMarkersSelected, oneDimensionSelected } = useStyleEditor(props.panel, objectFilter, objectMapper)
  ...
  const maxAngleMarkerRadiusPercent =
    SETTINGS.style.maxAngleMarkerRadiusPercent;
  const minAngleMarkerRadiusPercent =
