<template>
  <div>
    ...
    <table v-if="availableSessions.length > 0">
      <tr><th>Socket Id</th><th>Action</th></tr>
      <tr v-for="(sessId, pos) in availableSessions" :key="pos">
        <td>{{sessId}}</td>
        <td><v-btn @click="joinSession(sessId)">Join</v-btn></td>
      </tr>
    </table>
  </div>
</template>
...
export default class Sessions extends Vue {
  ...
  joinSession(session: string): void {
    ...
    this.socket = io(process.env.VUE_APP_SESSION_SERVER_URL || "http://localhost:4000");
