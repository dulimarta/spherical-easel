<template>
  ...
  <ul>
    ...
    <template v-if="myStudio">
      ...
      <li>
        Studio ID:
        <code>{{ myStudio!.id }}</code>
      </li>
      <li>
        Participants ({{ myStudio!.participants.length }}):
        {{ myStudio!.participants.join(", ") }}
      </li>
    </template>
    <li>
      Socket ID:
...
const { userDisplayedName, userEmail, userProfilePictureURL } =
