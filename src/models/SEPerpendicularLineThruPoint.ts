...
export class SEPerpendicularLineThruPoint extends SELine {
  ...
  public update(
    ...
    if (objectState && orderedSENoduleList) {
      if (objectState.has(this.id)) {
        // `Perpendicular Lint Thru Point with id ${this.id} has been visited twice proceed no further down this branch of the DAG. Hopefully this is because we are moving two or more SENodules a the same time in the MoveHandler.`
        return;
      }
  ...
  public get noduleDescription(): string {
    ...
    if (this.seParentOneDimensional instanceof SESegment) {
      oneDimensionalParentType = i18n.global.t("objects.segments", 3);
    } else if (this.seParentOneDimensional instanceof SELine) {
      oneDimensionalParentType = i18n.global.t("objects.lines", 3);
    ...
    return String(
      i18n.global.t(`objectTree.perpendicularLineThru`, {
