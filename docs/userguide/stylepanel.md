...
- Text(`<Text>`) - The text displayed by the text object.
- M2(Length(Li1)) - The length of line segment Li1, with Measurement Token M2.
- M3(Value(S1)) - The value of slider S1, with Measurement Token M3.
- M4(Calculation(`<Calculation>`)) - The value of the calculation with Measurement Token M4
...
- Pc - [Parametric Curve](/tools/advanced.html#parametric-curve-user-defined)
  ...
  <!--- M - [Measurement Token](/userguide/toolsobjectspanel.html#calculations) **(Not Modifiable)** -->
- S - [Slider](/tools/measurement.html#slider)
- Tx - [Text](/tools/basic.html#text)
- Am - Angle Marker

Maximum length is ?4? characters. TODO: Update? Is a limit even needed?
...
This allows the user to select the attributes of how the text of the name and caption are rendered. The options are
...
Which options are displayed depends on the objects selected. For example, the Fill Color is only available for points, circles, conic, and polygons. Size is only available for points and Dashing is only available for one-dimensional objects.
...
- For [Line Segments](/tools/basic.html#line-segment) and [Circular Arc](/tools/advanced.html#circular-arc) the extension value is set here. The user can enter a fix number or a Measurement Token to control the lenght of the Line Segment or Circular Arc. Details can be found in the tool descriptions.
...
- For Calculations in the Objects tab, you can edit them here.
